...
class Ui_Form(QtWidgets.QDialog):
    ...
    def tableView_set(self):
        ...
        index = 0
        for i in range(0, 30):
            self.model.setItem(i, 0, QtWidgets.QStandardItem(_fromUtf8(set_value_type_list[index])))
            index += 1
            # 设置字符颜色
            self.model.item(i, 0).setForeground(QtWidgets.QBrush(QtWidgets.QColor(255, 0, 0)))
            # 设置字符位置
            self.model.item(i, 0).setTextAlignment(QtCore.Qt.AlignCenter)
            if index >= len(set_value_type_list):
                break

            self.model.setItem(i, 3, QtWidgets.QStandardItem(_fromUtf8(set_value_type_list[index])))
            index += 1
            # 设置字符颜色
            self.model.item(i, 3).setForeground(QtWidgets.QBrush(QtWidgets.QColor(255, 0, 0)))
            # 设置字符位置
            self.model.item(i, 3).setTextAlignment(QtCore.Qt.AlignCenter)
            if index >= len(set_value_type_list):
                break

            self.model.setItem(i, 6, QtWidgets.QStandardItem(_fromUtf8(set_value_type_list[index])))
            index += 1
            # 设置字符颜色
            self.model.item(i, 6).setForeground(QtWidgets.QBrush(QtWidgets.QColor(255, 0, 0)))
            # 设置字符位置
